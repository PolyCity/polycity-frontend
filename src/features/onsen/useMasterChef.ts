import { useActiveWeb3React, usePichiContract } from '../../hooks'
    ...
    async (pid: number) => {
      try {
        ...
        if (chef === Chef.MASTERCHEF) {
          ...
          const pendingPichi = await contract?.pendingPichi(pid, account)
