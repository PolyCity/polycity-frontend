...
import { PICHI } from '../../config/tokens'
...
export function usePendingPichi(farm) {
  ...
  const result = useSingleCallResult(args ? contract : null, 'pendingPichi', args)?.result

  const value = result?.[0]

  const amount = value ? JSBI.BigInt(value.toString()) : undefined

  return amount ? CurrencyAmount.fromRawAmount(PICHI[chainId], amount) : undefined
}
...
export function useChefPositions(contract?: Contract | null, rewarder?: Contract | null, chainId = undefined) {
  ...
  const pendingPichi = useSingleContractMultipleData(args ? contract : null, 'pendingPichi', args)
  ...
  return useMemo(() => {
    if (!pendingPichi || !userInfo) {
      return []
    }
    return zip(pendingPichi, userInfo)
      .map((data, i) => ({
        id: args[i][0],
        pendingPichi: data[0].result?.[0] || Zero,
        amount: data[1].result?.[0] || Zero,
        chef: getChef(),
        // pendingTokens: data?.[2]?.result,
      }))
      .filter(({ pendingPichi, amount }) => {
        return (pendingPichi && !pendingPichi.isZero()) || (amount && !amount.isZero())
      })
  }, [args, getChef, pendingPichi, userInfo])
}
