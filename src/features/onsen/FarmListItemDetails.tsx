...
import { ChainId, MASTERCHEF_ADDRESS, MASTERCHEF_V2_ADDRESS, MINICHEF_ADDRESS, Token, ZERO } from '@polycity/sdk'
import { Chef, PairType } from './enum'
import { Disclosure, Transition } from '@headlessui/react'
import React, { useState } from 'react'
import { usePendingPichi, useUserInfo } from './hooks'
...
const FarmListItem = ({ farm }) => {
  ...
  const liquidityToken = new Token(
    ...
    farm.pair.type === PairType.KUSHO ? Number(farm.pair.asset.decimals) : 18,
    farm.pair.symbol,
    farm.pair.name
  )
  ...
  const pendingPichi = usePendingPichi(farm)
  ...
  return (
    ...
    >
      <Disclosure.Panel className="flex flex-col w-full border-t-0 rounded rounded-t-none bg-dark-800" static>
        ...
        {pendingPichi && pendingPichi.greaterThan(ZERO) && (
          <div className="px-4 pb-4">
            ...
            >
              {i18n._(t`Harvest ${formatNumber(pendingPichi.toFixed(18))} PICHI ${
