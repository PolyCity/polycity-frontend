...
const MeowshiButton: FC<MeowshiButtonProps> = ({ meowshiState }) => {
  ...
  const sushiBalance = useTokenBalance(account, PICHI[ChainId.MAINNET])
  const xPichiBalance = useTokenBalance(account, XPICHI)
  const { approvalState, approve, meow, unmeow, meowPichi, unmeowPichi } = useMeowshi(
  ...
  const handleSubmit = async () => {
    ...
    let tx
    if (doMeow) {
      if (currencies[Field.INPUT]?.symbol === 'PICHI') {
        tx = await meowPichi({
          value: parseUnits(fields[Field.INPUT], sushiBalance.currency.decimals),
          decimals: sushiBalance.currency.decimals,
        })
      }
      if (currencies[Field.INPUT]?.symbol === 'xPICHI') {
        tx = await meow({
          value: parseUnits(fields[Field.INPUT], sushiBalance.currency.decimals),
          decimals: xPichiBalance.currency.decimals,
        })
      }
    } else {
      if (currencies[Field.OUTPUT]?.symbol === 'PICHI') {
        tx = await unmeowPichi({
          value: parseUnits(fields[Field.INPUT], sushiBalance.currency.decimals),
          decimals: xPichiBalance.currency.decimals,
        })
      }
      if (currencies[Field.OUTPUT]?.symbol === 'xPICHI') {
        tx = await unmeow({
          value: parseUnits(fields[Field.INPUT], sushiBalance.currency.decimals),
          decimals: xPichiBalance.currency.decimals,
        })
