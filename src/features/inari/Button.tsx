...
const InariButton: FC<InariButtonProps> = ({ children, ...rest }) => {
  ...
  const { balances, execute, antiqueApproveCallback, approveCallback } = useSelectedInariStrategy()
  ...
  const handleGetPermit = useCallback(async () => {
    await antiqueApproveCallback.getPermit()
  }, [antiqueApproveCallback])

  // Add token approve to approve flow
  // Note that this is not required when unstaking from AntiqueBox strategies, hence approveCallback can be null
  const steps = []
  if (approveCallback) steps.push(approveCallback[0] === ApprovalState.APPROVED)
  if (antiqueApproveCallback) steps.push(antiqueApproveCallback.approvalState === AntiqueApprovalState.APPROVED)
  ...
  if (antiqueApproveCallback && antiqueApproveCallback.approvalState === AntiqueApprovalState.PENDING)
    return (
      <>
        <Button {...rest} disabled color="gray">
  ...
  if (antiqueApproveCallback && antiqueApproveCallback.approvalState === AntiqueApprovalState.NOT_APPROVED)
    return (
      <>
