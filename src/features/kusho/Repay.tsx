...
import {
  Percent,
  POLYCITYDEX_MULTISWAPPER_ADDRESS,
  POLYCITYDEX_MULTI_EXACT_SWAPPER_ADDRESS,
  WNATIVE,
} from '@polycity/sdk'
...
import { toAmount, toShare } from '../../functions/antiquebox'
import {
...
import { KushoCooker } from '../../entities'
...
const DEFAULT_KUSHO_REPAY_SLIPPAGE_TOLERANCE = new Percent(5, 100)
...
export default function Repay({ pair }: RepayProps) {
  ...
  // State
  const [useAntiqueRepay, setUseAntiqueRepay] = useState<boolean>(pair.asset.antiqueBalance.gt(0))
  const [useAntiqueRemove, setUseAntiqueRemoveCollateral] = useState<boolean>(true)
  ...
  const balance = useAntiqueRepay
    ? toAmount(pair.asset, pair.asset.antiqueBalance)
    : assetNative
    ? info?.ethBalance
    : pair.asset.balance
  ...
  const allowedSlippage = useUserSlippageToleranceWithDefault(DEFAULT_KUSHO_REPAY_SLIPPAGE_TOLERANCE)
  ...
  const warnings = new Warnings()
    .addError(
      assetNative && !useAntiqueRepay && pinRepayMax,
      `You cannot MAX repay ${pair.asset.tokenInfo.symbol} directly from your wallet. Please deposit your ${pair.asset.tokenInfo.symbol} into the AntiqueBox first, then repay. Because your debt is slowly accrueing interest we can't predict how much it will be once your transaction gets mined.`
    )
    .addError(
    ...
    .addError(
      ...
      new Warning(
        ...
        `Please make sure your ${
          useAntiqueRepay ? 'AntiqueBox' : 'wallet'
  ...
  async function onExecute(cooker: KushoCooker) {
    ...
    if (swap && trade) {
      ...
      cooker.antiqueTransferCollateral(pair.userCollateralShare, POLYCITYDEX_MULTI_EXACT_SWAPPER_ADDRESS[chainId || 1])
      ...
      cooker.action(
        POLYCITYDEX_MULTI_EXACT_SWAPPER_ADDRESS[chainId || 1],
        ZERO,
      ...
      if (!useAntiqueRemove) {
        cooker.antiqueWithdrawCollateral(ZERO, BigNumber.from(-1))
      }

      summary = 'Repay All'
    } else {
      if (pinRepayMax && pair.userBorrowPart.gt(0) && balance.gte(pair.currentUserBorrowAmount.value)) {
        cooker.repayPart(pair.userBorrowPart, useAntiqueRepay)
        summary = 'Repay Max'
      } else if (displayRepayValue.toBigNumber(pair.asset.tokenInfo.decimals).gt(0)) {
        cooker.repay(displayRepayValue.toBigNumber(pair.asset.tokenInfo.decimals), useAntiqueRepay)
        summary = 'Repay'
      }
      if (
        ...
        cooker.removeCollateral(share, useAntiqueRemove)
  ...
  return (
    <>
      ...
      <SmartNumberInput
        ...
        useAntiqueTitle={`Repay ${pair.asset.tokenInfo.symbol} from`}
        useAntique={useAntiqueRepay}
        setUseAntique={setUseAntiqueRepay}
        maxTitle="Balance"
        max={balance}
        pinMax={pinRepayMax}
      ...
      <SmartNumberInput
        ...
        useAntiqueTitle={`Remove ${pair.collateral.tokenInfo.symbol} to`}
        useAntique={useAntiqueRemove}
        setUseAntique={setUseAntiqueRemoveCollateral}
      ...
      <KushoApproveButton
        color="pink"
        content={(onCook: any) => (
          <TokenApproveButton value={displayRepayValue} token={assetToken} needed={!useAntiqueRepay}>
