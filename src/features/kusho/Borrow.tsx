...
import { Percent, POLYCITYDEX_MULTISWAPPER_ADDRESS, WNATIVE } from '@polycity/sdk'
...
export default function Borrow({ pair }: BorrowProps) {
  ...
  // State
  const [useAntiqueCollateral, setUseAntiqueCollateral] = useState<boolean>(pair.collateral.antiqueBalance.gt(0))
  const [useAntiqueBorrow, setUseAntiqueBorrow] = useState<boolean>(true)
  ...
  const collateralBalance = useAntiqueCollateral
    ? pair.collateral.antiqueBalance
    : assetNative
    ? info?.ethBalance
  ...
  collateralWarnings.add(
    ...
    `Please make sure your ${
      useAntiqueCollateral ? 'AntiqueBox' : 'wallet'
  ...
  async function onExecute(cooker: KushoCooker): Promise<string> {
    ...
    if (borrowValueSet) {
      ...
      if (swap && !useAntiqueCollateral) {
        cooker.antiqueDepositCollateral(collateralValue.toBigNumber(pair.collateral.tokenInfo.decimals))
      }

      cooker.borrow(
        borrowValue.toBigNumber(pair.asset.tokenInfo.decimals),
        swap || useAntiqueBorrow,
        swap ? POLYCITYDEX_MULTISWAPPER_ADDRESS[chainId || 1] : ''
      )
    }
    if (borrowValueSet && trade) {
      ...
      cooker.action(
        POLYCITYDEX_MULTISWAPPER_ADDRESS[chainId || 1],
        ZERO,
  ...
  return (
    <>
      ...
      <SmartNumberInput
        ...
        useAntiqueTitle={`Add ${pair.collateral.tokenInfo.symbol} collateral from`}
        useAntique={useAntiqueCollateral}
        setUseAntique={setUseAntiqueCollateral}
        maxTitle="Balance"
      ...
      <SmartNumberInput
        ...
        useAntiqueTitle={`Borrow ${pair.asset.tokenInfo.symbol} to`}
        useAntique={useAntiqueBorrow}
        setUseAntique={setUseAntiqueBorrow}
        maxTitle="Max"
      ...
      <KushoApproveButton
        color="pink"
        content={(onCook: any) => (
          <TokenApproveButton value={collateralValue} token={collateralToken} needed={!useAntiqueCollateral}>
