import { ChainId, Currency, KUSHO_ADDRESS, NATIVE, Token, USDC_ADDRESS, WNATIVE, WNATIVE_ADDRESS } from '@polycity/sdk'
...
import {
  ...
  interestAccrue,
  takeFee,
} from '../../../functions/kusho'
import { toAmount, toShare } from '../../../functions/antiquebox'
import { useAntiqueBoxContract, useBoringHelperContract } from '../../../hooks/useContract'
...
export interface KushoContextProps {
  state: State
  dispatch: React.Dispatch<any>
}

type KushoProviderProps = {
  state: State
  dispatch: React.Dispatch<any>
}

export const KushoContext = createContext<{
  state: State
  dispatch: React.Dispatch<any>
}>({
  state: initialState,
...
async function getPairs(antiqueBoxContract, chainId: ChainId) {
  let logs = []
  let success = false
  const masterAddress = KUSHO_ADDRESS[chainId]
  if (chainId !== ChainId.BSC && chainId !== ChainId.MATIC) {
    logs = await antiqueBoxContract.queryFilter(antiqueBoxContract.filters.LogDeploy(masterAddress))
    success = true
  }
  if (!success) {
    logs = (
...
export function KushoProvider({ children }) {
  ...
  const updatePairs = useCallback(async () => {
    ...
    if (boringHelperContract && antiqueBoxContract) {
      // // console.log('READY TO RUMBLE')
      const info = rpcToObj(await boringHelperContract.getUIInfo(account, [], currency, [KUSHO_ADDRESS[chainId]]))

      // Get the deployed pairs from the logs and decode
      const logPairs = await getPairs(antiqueBoxContract, chainId)
      ...
      const pairs = rpcToObj(await boringHelperContract.pollKushoPairs(account, allPairAddresses))
      ...
      // Get balances, antiquebox info and allowences for the tokens
  ...
  }, [account, chainId, boringHelperContract, antiqueBoxContract, currency, tokens, wnative])
...
export function useKushoInfo() {
  const context = useContext(KushoContext)
  if (context === undefined) {
    throw new Error('useKushoInfo must be used within a KushoProvider')
  }
...
export function useKushoPairs() {
  const context = useContext(KushoContext)
  if (context === undefined) {
    throw new Error('useKushoPairs must be used within a KushoProvider')
  }
  return context.state.pairs
}

export function useKushoPair(address: string) {
  const context = useContext(KushoContext)
  if (context === undefined) {
    throw new Error('useKushoPair must be used within a KushoProvider')
  }
