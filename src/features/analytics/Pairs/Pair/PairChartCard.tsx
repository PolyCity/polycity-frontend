import { useBlock, useDayData, usePichiPairs } from '../../../../services/graph'
...
export default function PairChartCard(props: PairChartCardProps): JSX.Element {
  ...
  const pair = usePichiPairs({ subset: [props.pair], shouldFetch: !!props.pair })?.[0]
  const pair1d = usePichiPairs({ subset: [props.pair], block: block1d, shouldFetch: !!props.pair && !!block1d })?.[0]
  const pair2d = usePichiPairs({ subset: [props.pair], block: block2d, shouldFetch: !!props.pair && !!block2d })?.[0]
