...
import {
  useFarms,
  useAntiqueUserTokens as useGetAntiqueUserTokens,
  useUserKushoPairs as useGetUserKushoPairs,
  useKushoPairs,
...
export function useUserPairs(chainId = undefined) {
  ...
  const pairsFiltered = usePichiPairs({
...
export function useUserFarms(chainId = undefined) {
  ...
  const sushiPairs = usePichiPairs({ subset: farmAddresses, chainId, shouldFetch: !!farmAddresses })
  const kushoPairs = useKushoPairs({ subset: farmAddresses, chainId, shouldFetch: !!farmAddresses })

  const nativePrice = useNativePrice({ chainId })

  const userFarmsFormatted = useMemo(
    () =>
      userPositions && farms && (sushiPairs || kushoPairs)
        ? userPositions
            .map((position) => {
              const farm = farms.find((f) => f.chef === position.chef && f.id === position.id)
              if (!farm) return undefined
              const sushiPair = sushiPairs?.find((pair) => pair.id === farm.pair)
              const kushoPair = kushoPairs?.find((pair) => pair.id === farm.pair)

              if (!sushiPair && !kushoPair) return undefined

              return {
                ...farm,
                type: sushiPair ? 'lp' : kushoPair ? 'kusho' : 'unknown',
                ...
                userStakedUSD: sushiPair
                  ...
                  : (position.amount / getFraction(kushoPair) / 10 ** kushoPair.token0.decimals) *
                    kushoPair.token0.derivedETH *
                    nativePrice,
                chain: ChainId[chainId],
              }
            })
...
export function useAllAntiqueUserTokens() {
  let tokens = []
  for (const chainId of chainsWithFeature(Feature.ANTIQUEBOX)) {
    tokens = [...tokens, ...useAntiqueUserTokens(ChainId[chainId])]
  }
  return tokens
}

export function useAntiqueUserTokens(chainId = undefined) {
  const { chainId: chainIdSelected } = useActiveWeb3React()
  chainId = chainId ?? chainIdSelected

  const antiqueUserTokens = useGetAntiqueUserTokens(undefined, chainId)
  ...
  return useMemo(() => {
    return antiqueUserTokens && tokens && nativePrice
      ? antiqueUserTokens.map((antiqueToken) => {
          const token = tokens.find((t) => t.id === antiqueToken.id)

          return {
            ...antiqueToken,
            valueUSD: (antiqueToken.amount * token?.derivedETH * nativePrice) / 10 ** token?.decimals ?? 0,
            chain: ChainId[chainId],
          }
        })
      : []
  }, [antiqueUserTokens, tokens, nativePrice])
}

export function useAllUserKushoPairs() {
  let pairs = []
  for (const chainId of chainsWithFeature(Feature.ANTIQUEBOX)) {
    pairs = [...pairs, ...useUserKushoPairs(ChainId[chainId])]
  }
  return pairs
}

export function useUserKushoPairs(chainId = undefined) {
  const userKushoPairs = useGetUserKushoPairs(undefined, chainId)

  const kushoTokenAddresses = useMemo(
    () =>
      userKushoPairs
        ? uniq([
            ...userKushoPairs.map((userPair) => userPair.pair.asset.id),
            ...userKushoPairs.map((userPair) => userPair.pair.collateral.id),
          ])
        : [],
    [userKushoPairs]
  )

  const tokens = useTokens({ subset: kushoTokenAddresses, chainId, shouldFetch: !!kushoTokenAddresses })

  const nativePrice = useNativePrice({ chainId })

  return useMemo(() => {
    return userKushoPairs && tokens && nativePrice
      ? userKushoPairs.map((userPair) => {
      ...
      : []
  }, [userKushoPairs, tokens, nativePrice])
}
