...
const PayFromToggle: FC = () => {
  ...
  const handleChange = (checked: boolean) => {
    dispatch(setFromAntiqueBalance(!checked))
  }
      ...
      <Switch.Group>
        <div className="flex items-center">
          <Switch.Label className="mr-2">
            <Typography variant="sm" className={fromAntiqueBalance ? 'text-primary' : 'text-secondary'}>
              AntiqueBox
            </Typography>
          </Switch.Label>
          <Switch
            checked={!fromAntiqueBalance}
            onChange={handleChange}
            className="bg-gray-600 relative inline-flex items-center h-3 rounded-full w-9 transition-colors"
          >
            <span
              className={`${
                !fromAntiqueBalance ? 'translate-x-5' : ''
              } inline-block w-4 h-4 transform bg-white rounded-full transition-transform`}
            />
          </Switch>
          <Switch.Label className="ml-2">
            <Typography variant="sm" className={!fromAntiqueBalance ? 'text-primary' : 'text-low-emphesis'}>
              {i18n._(t`Wallet`)}
            </Typography>
          </Switch.Label>
        </div>
