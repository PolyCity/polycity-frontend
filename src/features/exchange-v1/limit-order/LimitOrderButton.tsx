...
import { ANTIQUEBOX_ADDRESS, ChainId, Currency } from '@polycity/sdk'
import Button, { ButtonProps } from '../../../components/Button'
import { Field, setFromAntiqueBalance } from '../../../state/limit-order/actions'
import React, { FC, useCallback, useState } from 'react'
import { useAddPopup, useWalletModalToggle } from '../../../state/application/hooks'
import { useDerivedLimitOrderInfo, useLimitOrderState } from '../../../state/limit-order/hooks'
import useLimitOrderApproveCallback, { AntiqueApprovalState } from '../../../hooks/useLimitOrderApproveCallback'

import Alert from '../../../components/Alert'
import { AppDispatch } from '../../../state'
import ConfirmLimitOrderModal from './ConfirmLimitOrderModal'
import Dots from '../../../components/Dots'
import { LimitOrder } from '@polycity/sdk'
...
const LimitOrderButton: FC<LimitOrderButtonProps> = ({ currency, color, ...rest }) => {
  ...
  const { fromAntiqueBalance, orderExpiration, recipient } = useLimitOrderState()
  ...
  const showLimitApprove =
    (approvalState === AntiqueApprovalState.NOT_APPROVED || approvalState === AntiqueApprovalState.PENDING) && !permit
  ...
  const disabled =
    !!inputError ||
    approvalState === AntiqueApprovalState.PENDING ||
    depositPending ||
  ...
  if (depositPending)
    button = (
      <Button disabled={disabled} color={disabled ? 'gray' : color} onClick={deposit} {...rest}>
        <Dots>{i18n._(t`Depositing ${currency.symbol} into AntiqueBox`)}</Dots>
      </Button>
    )
  else if (!account)
    button = (
    ...
    (permit && !fromAntiqueBalance) ||
    (!permit && approvalState === AntiqueApprovalState.APPROVED && !fromAntiqueBalance)
  )
    button = (
      <Button disabled={disabled} color={disabled ? 'gray' : 'blue'} onClick={deposit} {...rest}>
        {i18n._(t`Deposit ${currency.symbol} into AntiqueBox`)}
      </Button>
    )
