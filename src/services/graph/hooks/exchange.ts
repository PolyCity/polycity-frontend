...
export function usePichiPrice(variables = undefined, swrConfig: SWRConfiguration = undefined) {
  const { data } = useSWR(['sushiPrice', JSON.stringify(variables)], () => getPichiPrice(variables), swrConfig)
  return data
}
