...
import { ChainId, Currency, Token } from '@polycity/sdk'
import { MEOW, PICHI, XPICHI } from '../../config/tokens'
...
import useMeowshiPerXPichi from '../../hooks/useMeowshiPerXPichi'
...
export default function Meowshi() {
  ...
  const sushiPerXPichi = usePichiPerXPichi()
  const [meowshiPerXPichi, xPichiPerMeowshi] = useMeowshiPerXPichi()
  ...
  const [currencies, setCurrencies] = useState({
    [Field.INPUT]: PICHI[ChainId.MAINNET],
    [Field.OUTPUT]: MEOW,
  })
  ...
  const handleInput = useCallback(
    async (val, field) => {
      setFields((prevState) => {
        const inputRate =
          currencies[Field.INPUT] === XPICHI
            ? meowshiPerXPichi.mul(e10(5))
            : meowshiPerXPichi.mul(e10(5)).mulDiv(e10(18), sushiPerXPichi.toString().toBigNumber(18))
        const outputRate =
          currencies[Field.OUTPUT] === XPICHI
            ? xPichiPerMeowshi.div(e10(5))
            : xPichiPerMeowshi.mulDiv(sushiPerXPichi.toString().toBigNumber(18), e10(18)).div(e10(5))
    ...
    [currencies, meowshiPerXPichi, sushiPerXPichi, xPichiPerMeowshi]
  )
  ...
  return (
    <Container id="meowshi-page" className="py-4 md:py-8 lg:py-12" maxWidth="2xl">
      <Head>
        <title>Meowshi | Pichi</title>
        <meta key="description" name="description" content="PolyCityDex Meowshi..." />
      </Head>
      ...
      <div className="z-0 relative mb-[-38px] md:mb-[-54px] ml-0 md:ml-4 flex justify-between gap-6 items-center">
        ...
        <div className="bg-[rgba(255,255,255,0.04)] p-4 py-2 rounded flex flex-row items-center gap-4 mb-[54px]">
          ...
          <Typography variant="xs" weight={700}>
            {i18n._(t`MEOW tokens wrap xPICHI into AntiqueBox for double yields and can be
      ...
      <div className="relative grid gap-4 p-4 border-2 rounded z-1 bg-dark-900 shadow-swap border-dark-800">
        ...
        <div className="relative mt-[-24px] mb-[-24px] ml-[28px] flex items-center">
          ...
          <Typography variant="sm" className="text-secondary ml-[26px]">
            ...
            {(currencies[Field.INPUT] === PICHI[ChainId.MAINNET] ||
              currencies[Field.OUTPUT] === PICHI[ChainId.MAINNET]) &&
              ' xPICHI → '}
