...
import { BAR_ADDRESS, ZERO } from '@polycity/sdk'
...
import usePichiBar from '../../hooks/usePichiBar'
import { usePichiPrice } from '../../services/graph'
...
export default function Stake() {
  ...
  const sushiBalance = useTokenBalance(account ?? undefined, PICHI[ChainId.MAINNET])
  const xPichiBalance = useTokenBalance(account ?? undefined, XPICHI)

  const sushiPrice = usePichiPrice()

  const { enter, leave } = usePichiBar()

  const { data } = useSWR(`{bar(id: "0x8798249c2e607446efb7ad49ec89dd1865ff4272") {ratio, totalSupply}}`, fetcher)

  const xPichiPerPichi = parseFloat(data?.bar?.ratio)
  ...
  const balance = activeTab === 0 ? sushiBalance : xPichiBalance
  ...
  return (
    <Container id="bar-page" className="py-4 md:py-8 lg:py-12" maxWidth="full">
      <Head>
        <title key="title">Stake | Pichi</title>
        <meta
          key="description"
          name="description"
          content="Stake PICHI in return for xPICHI, an interest bearing and fungible ERC20 token designed to share revenue generated by all PICHI products."
        />
        <meta key="twitter:url" name="twitter:url" content="https://app.sushi.com/stake" />
        <meta key="twitter:title" name="twitter:title" content="STAKE PICHI" />
        <meta
        ...
        <meta key="og:title" property="og:title" content="STAKE PICHI" />
          ...
          <div className="flex flex-col w-full max-w-xl mt-auto mb-2">
            <div className="flex max-w-lg">
              <div className="self-end mb-3 text-lg font-bold md:text-2xl text-high-emphesis md:mb-7">
                {i18n._(t`Maximize yield by staking PICHI for xPICHI`)}
              </div>
              {/* <div className="self-start pl-6 pr-3 mb-1 min-w-max md:hidden">
                                <img src={XPichiSignSmall} alt="xsushi sign" />
                            </div> */}
            </div>
            <div className="max-w-lg pr-3 mb-2 text-sm leading-5 text-gray-500 md:text-base md:mb-4 md:pr-0">
              {i18n._(t`For every swap on the exchange on every chain, 0.05% of the swap fees are distributed as PICHI
                                proportional to your share of the PichiBar. When your PICHI is staked into the PichiBar, you receive
                                xPICHI in return for voting rights and a fully composable token that can interact with other protocols.
                                Your xPICHI is continuously compounding, when you unstake you will receive all the originally deposited
                                PICHI and any additional from fees.`)}
            </div>
            {/* <div className="flex">
                            ...
                            <div>
                                <StyledLink className="text-sm text-lg whitespace-nowrap md:text-lg md:leading-5">
                                    Tips for using xPICHI
                                </StyledLink>
                            </div>
                        </div> */}
          </div>
          <div className="hidden px-8 ml-6 md:block w-72">
            <Image src="/xsushi-sign.png" alt="xPICHI sign" width="100%" height="100%" layout="responsive" />
          </div>
        </div>
              ...
              <div className="w-full max-w-xl px-3 pt-2 pb-6 rounded bg-dark-900 md:pb-9 md:pt-4 md:px-8">
                ...
                <div className="flex items-center justify-between w-full mt-6">
                  <p className="font-bold text-large md:text-2xl text-high-emphesis">
                    {activeTab === 0 ? i18n._(t`Stake PICHI`) : i18n._(t`Unstake`)}
                  </p>
                  <div className="border-gradient-r-pink-red-light-brown-dark-pink-red border-transparent border-solid border rounded-3xl px-4 md:px-3.5 py-1.5 md:py-0.5 text-high-emphesis text-xs font-medium md:text-base md:font-normal">
                    {`1 xPICHI = ${xPichiPerPichi.toFixed(4)} PICHI`}
                  </div>
                </div>
                ...
                <div className="relative w-full h-0 pointer-events-none bottom-14">
                  ...
                  >
                    <div className="flex space-x-2 ">
                      ...
                      >
                        {`${input ? input : '0'} ${activeTab === 0 ? '' : 'x'}PICHI`}
                      </p>
                    </div>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex flex-col justify-center">
                      <p className="text-sm font-bold md:text-lg text-high-emphesis">
                        {xPichiBalance ? xPichiBalance.toSignificant(4) : '-'}
                      </p>
                      <p className="text-sm md:text-base text-primary">xPICHI</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col flex-grow">
                  ...
                  <div className="flex items-center ml-8 space-x-4 md:ml-0">
                    ...
                    <div className="flex flex-col justify-center">
                      ...
                      <p className="text-sm md:text-base text-primary">PICHI</p>
                    </div>
                  </div>
                </div>
