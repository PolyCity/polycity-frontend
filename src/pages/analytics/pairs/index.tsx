import { useBlock, usePichiPairs } from '../../../services/graph'
...
export default function Pairs() {
  ...
  const pairs = usePichiPairs()
  const pairs1d = usePichiPairs({ block: block1d, shouldFetch: !!block1d })
  const pairs2d = usePichiPairs({ block: block2d, shouldFetch: !!block2d && type !== 'all' }) // No need to fetch if we don't need the data
  const pairs1w = usePichiPairs({ block: block1w, shouldFetch: !!block1w })
  const pairs2w = usePichiPairs({ block: block2w, shouldFetch: !!block2w && type !== 'all' })
