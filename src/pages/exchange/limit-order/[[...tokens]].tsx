import { ChainId, Token } from '@polycity/sdk'
...
function LimitOrder() {
  ...
  // Limit order state
  const { independentField, typedValue, recipient, limitPrice, fromAntiqueBalance } = useLimitOrderState()

  // Limit order derived state
  const { currencies, parsedAmounts, walletBalances, antiqueboxBalances, currentPrice } = useDerivedLimitOrderInfo()
  ...
  const atMaxAmountInput = Boolean(
    fromAntiqueBalance
      ? parsedAmounts[Field.INPUT]?.equalTo(antiqueboxBalances[Field.INPUT])
  ...
  }, [antiqueboxBalances, fromAntiqueBalance, maxAmountInput, onUserInput])
  ...
  return (
    <Container id="limit-order-page" className="py-4 space-y-6 md:py-8 lg:py-12" maxWidth="2xl">
      <Head>
        <title>{i18n._(t`Limit order`)} | Pichi</title>
        <meta
          name="description"
          content="PolyCityDex allows for swapping of ERC20 compatible tokens across multiple networks"
        />
      </Head>
      <TokenWarningModal
