...
export default function Farm(): JSX.Element {
  ...
  const swapPairs = usePichiPairs({ subset: pairAddresses, shouldFetch: !!pairAddresses })

  const kushoPairs = useKushoPairs({ subset: pairAddresses })
  ...
  const map = (pool) => {
    ...
    const tvl = swapPair
      ...
      : balance * kushoPair.token0.derivedETH * ethPrice
  ...
  const FILTER = {
    ...
    kusho: (farm) => farm.pair.type === PairType.KUSHO && farm.allocPoint !== '0',
  ...
  return (
    <Container id="farm-page" className="grid h-full grid-cols-4 py-4 mx-auto md:py-8 lg:py-12 gap-9" maxWidth="7xl">
      <Head>
        <title>Farm | Pichi</title>
        <meta key="description" name="description" content="Farm PICHI" />
      </Head>
