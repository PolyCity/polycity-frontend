import { Currency, CurrencyAmount, Token } from '@polycity/sdk'
...
export default function Vesting() {
  ...
  return (
    <Container id="vesting-page" className="py-4 md:py-8 lg:py-12 m-auto w-full max-w-[900px]">
      <Head>
        <title>Vesting | Pichi</title>
        <meta key="description" name="description" content="PolyCityDex vesting..." />
      </Head>
      <div className="flex px-0 sm:px-4 md:flex-row md:space-x-10 lg:space-x-20 md:px-10">
        ...
        <div className="flex flex-col gap-3 max-w-[400px]">
          <div className="relative w-full overflow-hidden rounded bg-dark-900">
            <div className="flex flex-col gap-3 p-4">
              <div className="flex flex-row justify-between">
                <div className="font-bold text-white">{i18n._(t`Your Claimable PICHI this Week`)}</div>
                <QuestionHelper text="Your Vested PICHI will be released each week for the next 6 months. The amount released each week is determined by your historical farming rewards. You do not need to harvest each week as unclaimed amounts from each week will continue to accrue onto the next." />
              </div>
              ...
              <div className="flex flex-col items-baseline">
                ...
                {account ? (
                  <div className="text-sm text-secondary">
                    {totalLocked ? (
                      i18n._(t`Historical Total Locked: ${formatNumber(totalLocked)} PICHI`)
                    ) : (
              ...
              >
                {pendingTreasurySignature ? (
                  ...
                  <> {claimConfirmed ? i18n._(t`Claimed`) : i18n._(t`Claim PICHI`)}</>
                )}
          ...
          <div className="relative w-full overflow-hidden rounded bg-dark-900">
            <div className="flex flex-col gap-3 p-4">
              <div className="font-bold text-white">{i18n._(t`Things you can do with your PICHI`)}</div>
              <div className="p-4 rounded bg-dark-800">
                <Link href="/stake">
                  <a className="flex items-center justify-between gap-3">
                    <div className="flex flex-col gap-1">
                      <div className="font-bold text-white">{i18n._(t`Stake PICHI for xPICHI`)}</div>
                      <div className="text-sm text-secondary">
                        {t`Gain governance rights with xPICHI and earn 5% APR (0.05% of
                ...
                <Link href={`/saave`}>
                  <a className="flex items-center justify-between gap-3">
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="text-sm text-secondary">
                        {t`Stake into xPICHI add collateral as axPICHI on Aave all in
                                                            one click`}
                      </div>
                    </div>
                    <div className="min-w-[32px]">
              ...
              <div className="p-4 rounded bg-dark-800">
                <div className="flex flex-col gap-1">
                  <div className="font-bold text-white">{i18n._(t`Deposit PICHI into AntiqueBox`)}</div>
                  <div className="text-sm text-secondary">
