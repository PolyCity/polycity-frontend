import { CHAINLINK_ORACLE_ADDRESS, Currency, KUSHO_ADDRESS } from '@polycity/sdk'
...
import { useAntiqueBoxContract } from '../../../hooks/useContract'
...
function Create() {
  ...
  const handleCreate = async () => {
    try {
      ...
      const kushoData = defaultAbiCoder.encode(
      ...
      const tx = await antiqueBoxContract?.deploy(chainId && KUSHO_ADDRESS[chainId], kushoData, true)

      addTransaction(tx, {
        summary: `Add Kusho market ${currencies[Field.ASSET].symbol}/${currencies[Field.COLLATERAL].symbol} Chainlink`,
      })
  ...
  return (
    <>
      <Head>
        <title>Create Lending Pair | Kusho by Pichi</title>
        <meta key="description" name="description" content="Create Lending Pair on Kusho by Pichi" />
      </Head>
      <Card
    ...
    <Layout
      left={
        <Card
          ...
          title={i18n._(t`Create a new Kusho Market`)}
          description={i18n._(
