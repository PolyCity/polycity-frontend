import { Borrow, Repay } from '../../../features/kusho'
import Provider, { useKushoInfo, useKushoPair } from '../../../features/kusho/context'
...
function Pair() {
  ...
  const pair = useKushoPair(router.query.pair as string)
  ...
  return (
    <>
      <Head>
        <title>{i18n._(t`Borrow ${pair?.asset?.symbol}-${pair?.collateral?.symbol}`)} | Pichi</title>
        <meta
          key="description"
          name="description"
          content={`Borrow ${pair?.asset?.symbol} against ${pair?.collateral?.symbol} collateral on Kusho by Pichi`}
        />
      </Head>
      <Card
      ...
      right={
        <Card className="h-full bg-dark-900">
          <div className="flex-col space-y-2">
            ...
            <div className="flex justify-between pt-3">
              <div className="text-xl text-high-emphesis">{i18n._(t`AntiqueBox`)}</div>
            </div>
            <div className="flex justify-between">
              ...
              <div className="flex flex-row text-lg text-high-emphesis">
                ...
                <QuestionHelper
                  text={i18n._(
                    t`AntiqueBox strategies can create yield for your collateral tokens. This token does not yet have a strategy in the AntiqueBox.`
                  )}
                />
              </div>
            </div>
            {pair && pair.oracle.name === 'PolyCityDex' && (
              <>
                <div className="flex justify-between pt-3">
