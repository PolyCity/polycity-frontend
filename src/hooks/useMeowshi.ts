...
import { useMeowshiContract, usePichiBarContract, usePichiContract } from './useContract'
...
const useMeowshi = (sushi: boolean) => {
  ...
  const sushiContract = usePichiContract(true)
  const barContract = usePichiBarContract(true)
  ...
  const meowPichi = useCallback(
    async (amount: BalanceProps | undefined) => {
      if (amount?.value) {
        try {
          const tx = await meowshiContract?.meowPichi(account, amount?.value)
          addTransaction(tx, { summary: 'Enter Meowshi' })
          return tx
        } catch (e) {
          return e
  ...
  const unmeowPichi = useCallback(
    async (amount: BalanceProps | undefined) => {
      if (amount?.value) {
        try {
          const tx = await meowshiContract?.unmeowPichi(account, amount?.value)
          addTransaction(tx, { summary: 'Leave Meowshi' })
          return tx
        } catch (e) {
          return e
