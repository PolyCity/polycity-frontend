...
import ANTIQUEBOX_ABI from '../constants/abis/antiquebox.json'
...
import PICHI_ABI from '../constants/abis/sushi.json'
...
export function usePichiContract(withSignerIfPossible = true): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId && PICHI_ADDRESS[chainId], PICHI_ABI, withSignerIfPossible)
}
...
export function usePichiBarContract(withSignerIfPossible?: boolean): Contract | null {
  const { chainId } = useActiveWeb3React()
...
export function useAntiqueBoxContract(withSignerIfPossible?: boolean): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId && ANTIQUEBOX_ADDRESS[chainId], ANTIQUEBOX_ABI, withSignerIfPossible)
}
