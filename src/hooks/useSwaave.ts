...
import { usePichiBarContract, usePichiContract } from './useContract'
...
const usePichiBar = () => {
  ...
  const sushiContract = usePichiContract(true) // withSigner
  const barContract = usePichiBarContract(true) // withSigner
  ...
  const enter = useCallback(
    ...
    async (amount: string) => {
      try {
        ...
        return addTransaction(tx, { summary: 'Enter PichiBar' })
      } catch (e) {
        return e
      }
    },
  ...
  const leave = useCallback(
    ...
    async (amount: string) => {
      try {
        ...
        return addTransaction(tx, { summary: 'Leave PichiBar' })
      } catch (e) {
        console.error(e)
        return e
      }
