...
import { ChainId, Currency, CurrencyAmount, JSBI, NATIVE, Percent, Price, WNATIVE } from '@polycity/sdk'
...
import { useAntiqueBalances } from '../antiquebox/hooks'
...
export function useDerivedLimitOrderInfo(): {
  ...
  antiqueboxBalances: { [field in Field]?: CurrencyAmount<Currency> }
  inputError?: string
  ...
  const antiqueBoxBalances = useAntiqueBalances()
  const balance = useMemo(
    () => antiqueBoxBalances?.find((el) => el.address === inputCurrency?.wrapped.address),
    [antiqueBoxBalances, inputCurrency?.wrapped.address]
  )
  ...
  const antiqueboxBalances = {
    [Field.INPUT]: inputCurrency
      ? CurrencyAmount.fromRawAmount(inputCurrency, balance?.antiqueBalance ? balance.antiqueBalance : 0)
      : undefined,
    [Field.OUTPUT]: outputCurrency
      ? CurrencyAmount.fromRawAmount(outputCurrency, balance?.antiqueBalance ? balance.antiqueBalance : 0)
      : undefined,
  }
  ...
  const [balanceIn, amountIn] = [
    fromAntiqueBalance ? antiqueboxBalances[Field.INPUT] : walletBalances[Field.INPUT],
...
export function queryParametersToSwapState(chainId: ChainId, parsedQs: ParsedQs) {
  ...
  return {
    ...
    fromAntiqueBalance: parseBooleanFieldParameter(parsedQs.fromAntique),
