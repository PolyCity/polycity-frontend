import { KUSHO_ADDRESS, USDC_ADDRESS, WNATIVE_ADDRESS } from '@polycity/sdk'
import { useAntiqueBoxContract, useBoringHelperContract, useContract } from '../../hooks/useContract'
...
import { easyAmount } from '../../functions/kusho'
import { getAddress } from '@ethersproject/address'
import { toAmount } from '../../functions/antiquebox'
import { useActiveWeb3React } from '../../hooks/useActiveWeb3React'
import { useAllTokens } from '../../hooks/Tokens'
import { useSingleCallResult } from '../multicall/hooks'
import useTransactionStatus from '../../hooks/useTransactionStatus'

export interface AntiqueBalance {
  address: string
  name: string
  symbol: string
  decimals: number
...
export function useAntiqueBalances(): AntiqueBalance[] {
    ...
    return tokenAddresses
      .map((key: string, i: number) => {
        ...
        return {
          ...
          antiqueBalance: balanceData.result[0][i].antiqueBalance,
          wallet: easyAmount(
            token.address === weth ? uiData.result[0].ethBalance : balanceData.result[0][i].balance,
            full
          ),
          antique: easyAmount(toAmount(full, balanceData.result[0][i].antiqueBalance), full),
        }
      })
      .filter((token) => token.balance.gt('0') || token.antiqueBalance.gt('0'))
  }, [
    uiData.loading,
    uiData.error,
    uiData.result,
...
export function useAntiqueBalance(tokenAddress: string): {
  value: BigNumber
  decimals: number
} {
  ...
  const fetchAntiqueBalance = useCallback(async () => {
    const balances = await boringHelperContract?.getBalances(account, [tokenAddressChecksum])
    const decimals = await tokenContract?.decimals()

    const amount = BigNumber.from(balances[0].antiqueShare).isZero()
      ? BigNumber.from(0)
      : BigNumber.from(balances[0].antiqueBalance)
          .mul(BigNumber.from(balances[0].antiqueAmount))
          .div(BigNumber.from(balances[0].antiqueShare))
  ...
  useEffect(() => {
    if (account && antiqueBoxContract && boringHelperContract && tokenContract) {
      fetchAntiqueBalance()
    }
  }, [account, antiqueBoxContract, currentTransactionStatus, fetchAntiqueBalance, tokenContract, boringHelperContract])

  return balance
}

export function useAntiqueMasterContractAllowed(masterContract?: string, user?: string): boolean | undefined {
  const contract = useAntiqueBoxContract()
