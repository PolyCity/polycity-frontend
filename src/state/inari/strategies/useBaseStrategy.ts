...
import { AntiquePermit } from '../../../hooks/useAntiqueMasterApproveCallback'
...
export interface BaseStrategyHook extends Strategy {
  execute: (val: CurrencyAmount<Token>, permit?: AntiquePermit) => Promise<any>
  ...
  antiqueApproveCallback?: null
}

// Every strategy should use a BaseStrategy. Can also use useBaseAntiqueBoxStrategy or useBaseHasPermitTokenStrategy
...
const useBaseStrategy = ({ id, general, tokenDefinitions }: useBaseStrategyInterface): BaseStrategyHook => {
  ...
  // Look at useStakePichiToCreamStrategy for an example
  ...
  // This one is converting Pichi to xPichi and vice-versa.
  ...
  const calculateOutputFromInput = useCallback(
    (zapIn: boolean, inputValue: string, inputToken: Token, outputToken: Token) => {
      if (!sushiPerXPichi || !inputValue) return null

      return (
        zapIn
          ? inputValue.toBigNumber(18).mulDiv(e10(18), sushiPerXPichi.toString().toBigNumber(18))
          : inputValue.toBigNumber(18).mulDiv(sushiPerXPichi.toString().toBigNumber(18), e10(18))
      )?.toFixed(18)
    },
    [sushiPerXPichi]
  )
